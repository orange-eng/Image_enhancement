...
if __name__ == "__main__":
    ...
    parser.add_argument('--video_name', default="data/test/LR_Video/small5.mp4",type=str, help='test low resolution video name')
    ...
    output_sr_name = path + '/out_srf_' + str(UPSCALE_FACTOR) + '_' + VIDEO_NAME.split('.')[0] + '.avi'
    output_compared_name = path + '/compare_srf_' + str(UPSCALE_FACTOR) + '_' + VIDEO_NAME.split('.')[0] + '.avi'
    sr_video_writer = cv2.VideoWriter(output_sr_name, cv2.VideoWriter_fourcc(*'XVID'), fps, sr_video_size)
    compared_video_writer = cv2.VideoWriter(output_compared_name, cv2.VideoWriter_fourcc(*'XVID'), fps,
                                            compared_video_size)
    # read frame
